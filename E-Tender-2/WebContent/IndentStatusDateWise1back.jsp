...
{
  alert(pid);
  window.open("UpdateIndent.jsp?pid="+ pid + " ","popup","width=500,height=500")
}
</script>
...
if ( test1 == 'last') { document.emp.mode.value='Last'; }
 ...
 document.emp.action="IndentStatusDateWise.jsp";
 document.emp.submit();
}
function sub()
{
...
<%
 ...
 try 
 {
	 ...
	 rs = stmt.executeQuery("select inm.ind_no,to_char(inm.inddate,'dd-mon-yyyy'),inm.qty_rec,to_char(inm.delv_date,'dd-mon-yyyy'),inm.est_rate,inm.status,inm.item_id from indent_master inm where inm.inddate between '"+ dt + "' and '" + dt1 + "' order by inm.item_id");
 ...
 catch(Exception e) 
 {
  ...
  <%
	...
	%>        
	 ...
	 <th class=tabhead3>Delivered Date
	...
	<td class=tabhead1><%=rs2.getString(1) %></td>
