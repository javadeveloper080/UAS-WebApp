...
function Go1()
{
if(emp.cat.selectedIndex==0)
{
alert("Please Select Search Option");
return false;
}else if(emp.cat.selectedIndex==1)
{
emp.action="SupplierInfo.jsp";
emp.submit();
}else
{
emp.action="SupplierCatInfo.jsp";
emp.submit();
...
document.emp.action="SupplierInfo.jsp";
document.emp.submit();
}
...
<%
 ...
 try 
 {
	 ...
	 rs = stmt.executeQuery("select sup_id,name,gen,phno,address,email,remarks,scat from supplier_info where scat='" + scat + "' order by sup_id");
  ...
  <%
	...
	<td class=tabhead1><a style={cursor:hand} onclick="Update('<%=tid%>')">Update</a></td>
	   ...
	   <TR>
	     ...
	     <TD class=tabhead width="10%">Select Supplier Catagory :<select name=cat>
		 <option><--Select Supplier Catagory--></option>
    ...
    <option>THERMAL POWER PLANT</option>
		...
		<input type=submit value="Get Data" onclick="return Go1()"></TD>
